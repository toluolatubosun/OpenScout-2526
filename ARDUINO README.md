...
**Motor C (Front-Left Motor)**
- Arduino Pin 2 → L298N EN C (PWM)
- Arduino Pin 22 → L298N IN5
- Arduino Pin 24 → L298N IN6

**Motor D (Front-Right Motor)**
- Arduino Pin 6 → L298N EN D (PWM)
- Arduino Pin 28 → L298N IN7
- Arduino Pin 30 → L298N IN8
...
Connect ultrasonic sensors for obstacle detection:
...
**Function:** Sensors detect obstacles within 30cm range to prevent collisions.

#### Buzzer

Connect buzzer for audio feedback:
- Buzzer Signal → Pin 34
- VCC → 5V
- GND → GND

**Function:** Beeps for alerts or notifications.

#### LEDs

Connect four indicator LEDs:
- LED1 → Pin 47
- LED2 → Pin 49
- LED3 → Pin 51
- LED4 → Pin 53
- All LEDs: VCC → 5V through 220Ω resistor, GND → GND

**Function:** Visual indicators for system status or events.
